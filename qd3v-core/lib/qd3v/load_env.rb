...
proc do
  ...
  raise("Load env: unknown 'APP_ENV' environment var value: '#{env}'") unless %w[
  ...
  if %w[development test].include?(env)
    ...
    env_files_abs = env_files.map { File.join(Dir.pwd, _1) }

    $stderr.puts("[DOTENV] Loading env files: #{env_files}")
